...
const ActivityItem = React.memo(
  ({ activity, index }: { activity: MemberActivity; index: number }) => (
    <div
...
// Virtualized Activities List Component
const VirtualizedActivitiesList = React.memo(
  ({ activities }: { activities: MemberActivity[] }) => {
    const containerHeight = 400;
    const itemHeight = 100;

    const { visibleItems, totalHeight, offsetY, handleScroll } =
      useVirtualization(activities, {
        itemHeight,
        containerHeight,
        overscan: 5,
      });
        ...
        <div style={{ height: totalHeight, position: "relative" }}>
          ...
          >
            <div className="space-y-3">
              {visibleItems.map(({ item: activity, index }) => (
                <div
...
const RecentActivities = ({ limit = 50 }: RecentActivitiesProps) => {
  const { activities, loading, refreshActivities } = useRecentActivities(limit);
      ...
      <CardContent className="p-6 bg-gradient-to-b from-slate-800/20 to-slate-900/40">
        <div className="space-y-3">
          {loading ? (
            ...
            <VirtualizedActivitiesList activities={activities} />
          ) : (
            // Regular list for small activity lists
